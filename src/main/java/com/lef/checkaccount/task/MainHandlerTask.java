...
public class MainHandlerTask {
	...
	public RetVo execute(String dayStr) {
		...
		if (analysisRecord != null && analysisRecord.getStatus().equals(TaskCode.task_status_doing)) {
			return RetVo.getFailRet(dayStr + "日期的文件处理中，请稍后");
		}
		...
		try {
			switch (step) {
			case 1:
				syncUserInfoTask.execute(dayStr); // 同步用户注册信息
			case 2:
				rechargeTask.execute(dayStr); // 入金
			case 3:
				conCludeTask.execute(dayStr); // 客户成交单
			case 4:
				takeNowTask.execute(dayStr); // 出金
			case 5:
				tollTask.execute(dayStr); // 交易所收费单
